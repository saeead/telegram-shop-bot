"""SQLAlchemy persistence models for catalog, store, audit, commerce, and delivery state."""
...
class ProductFileModel(Base):
    ...
    __table_args__ = (UniqueConstraint("telegram_chat_id", "telegram_message_id", name="uq_product_file_telegram_message"),)
    id: Mapped[UUID] = mapped_column(primary_key=True)
...
class OrderModel(Base):
    ...
    __table_args__ = (UniqueConstraint("order_code", name="uq_orders_order_code"), UniqueConstraint("idempotency_key", name="uq_orders_idempotency_key"))
    id: Mapped[UUID] = mapped_column(primary_key=True)
    ...
    payments: Mapped[list[PaymentModel]] = relationship(back_populates="order", cascade="all, delete-orphan")
...
class PaymentModel(Base):
    ...
    __table_args__ = (UniqueConstraint("order_id", name="uq_payments_order_id"), UniqueConstraint("provider", "provider_reference", name="uq_payments_provider_reference"))
    id: Mapped[UUID] = mapped_column(primary_key=True)
    ...
    attempts: Mapped[list[PaymentAttemptModel]] = relationship(back_populates="payment", cascade="all, delete-orphan", order_by="PaymentAttemptModel.created_at")


class PaymentAttemptModel(Base):
    __tablename__ = "payment_attempts"
    __table_args__ = (UniqueConstraint("idempotency_key", name="uq_payment_attempts_idempotency_key"),)
    id: Mapped[UUID] = mapped_column(primary_key=True)
...
class DeliveryRecordModel(Base):
    ...
    file_id: Mapped[UUID] = mapped_column(ForeignKey("product_files.id", ondelete="CASCADE"), index=True)
...
__all__ = [
    "AuditLogModel", "Base", "DeliveryRecordModel", "OrderItemModel", "OrderModel",
    "PaymentAttemptModel", "PaymentModel", "ProductFileModel", "ProductIntakeModel",
    "ProductModel", "StorePublicationModel",
]
