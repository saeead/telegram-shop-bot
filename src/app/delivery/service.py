...
class DeliveryService:
    ...
    async def _deliver_product(
        ...
        existing = {
            record.file_id: record for record in await self._repository.list_for_order(order_id)
        }
    ...
    async def _summary(self, order_id: UUID) -> DeliverySummary:
        ...
        failed = sum(
            record.status in {DeliveryStatus.FAILED, DeliveryStatus.PARTIAL} for record in records
        )
