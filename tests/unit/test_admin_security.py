...
def test_malformed_admin_product_identifier_is_rejected_without_exception():
    assert _parse_uuid("not-a-uuid") is None
    assert _parse_uuid("") is None
    assert _parse_uuid("../etc/passwd") is None


def test_valid_admin_product_identifier_is_accepted():
    product_id = uuid4()
    assert _parse_uuid(str(product_id)) == product_id


def test_oversized_or_malformed_callback_is_rejected():
...
def test_callback_parser_does_not_turn_path_like_values_into_uuid():
